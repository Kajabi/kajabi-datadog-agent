...
This repo is a clone of https://github.com/heroku/log-iss, and is deployed to the Heroku app `kajabi-datadog-agent`.
...
While integrating Datadog, we found that their HTTP log ingestion endpoint was not able to scale to meet the volume of logs sent by Heroku's log drain, resulting in loss of ~20% of our log volume. While `heroku/log-iss` is not an officially supported Heroku product, it was suggested as a solution by Heroku support.
